...
function getOperatorConditionName() {
  ${KUBECTL_BINARY} get -n "${INSTALLED_NAMESPACE}" OperatorCondition -o name | grep hyperconverged-operator
}

function getOperatorConditionUpgradeable() {
  ${KUBECTL_BINARY} get -n "${INSTALLED_NAMESPACE}" "$1" -o yaml #-o json | jq '.status.conditions'
}
...
if isOperatorConditionSupported; then
  ...
  echo "Reading OperatorCondition ${name}"
  getOperatorConditionUpgradeable "${name}"
else
  echo "OperatorCondition is not supported"
fi
