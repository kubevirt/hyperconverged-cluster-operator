...
for hco_pod in $( ${CMD} get pods -n ${HCO_NAMESPACE} -l "name=hyperconverged-cluster-operator" --field-selector=status.phase=Running  --no-headers -o custom-columns=name:.metadata.name); do
...
Msg "Read the HCO operator and webhook logs, before it been deleted"
LOG_DIR="${ARTIFACT_DIR}/logs"
mkdir -p "${LOG_DIR}"
for pod in $(${CMD} get pod -n ${HCO_NAMESPACE} -l "name in (hyperconverged-cluster-operator,hyperconverged-cluster-webhook)" -o custom-columns=name:.metadata.name --no-headers); do
