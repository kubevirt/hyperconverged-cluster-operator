	...
	Describe("Reconcile NodesController", func() {
		...
		Context("Node Count Change", func() {
			It("Should send event if nodeInfo was changed", func() {
				...
				r := &ReconcileNodeCounter{
					...
					HandleHyperShiftNodeLabeling: func(_ context.Context, _ client.Client, _ string, _ logr.Logger) error {
						return nil
					},
				}
		...
		Context("HyperShift Node Labeling", func() {
			It("Should label worker node when shouldLabelNodes is true", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedNode.Labels).To(HaveKey(labelNodeRoleControlPlane))
				Expect(updatedNode.Labels[labelNodeRoleControlPlane]).To(Equal(hypershiftLabelValue))
			})
			...
			It("Should not label control plane node", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedNode.Labels[labelNodeRoleControlPlane]).To(Equal(""))
			})
			...
			It("Should skip labeling when shouldLabelNodes is false", func() {
				...
				err = cl.Get(context.TODO(), client.ObjectKey{Name: "worker-1"}, updatedNode)
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedNode.Labels).NotTo(HaveKey(labelNodeRoleControlPlane))
			})

			It("Should remove HyperShift label when shouldLabelNodes is false", func() {
				workerNode := &corev1.Node{
					ObjectMeta: metav1.ObjectMeta{
						Name: "worker-1",
						Labels: map[string]string{
							labelNodeRoleWorker:       "",
							labelNodeRoleControlPlane: hypershiftLabelValue,
						},
					},
				}
				...
				// Create a custom function that removes the label
				removeLabelFunc := func(ctx context.Context, cli client.Client, nodeName string, logger logr.Logger) error {
					node := &corev1.Node{}
					if err := cli.Get(ctx, client.ObjectKey{Name: nodeName}, node); err != nil {
						return err
					}
					if currentValue, hasLabel := node.Labels[labelNodeRoleControlPlane]; hasLabel && currentValue == hypershiftLabelValue {
						updatedNode := node.DeepCopy()
						delete(updatedNode.Labels, labelNodeRoleControlPlane)
						return cli.Update(ctx, updatedNode)
					}
					return nil
				}
				...
				r := &ReconcileNodeCounter{
					...
					HandleHyperShiftNodeLabeling: removeLabelFunc,
				}
				...
				// Verify the HyperShift label was removed
				updatedNode := &corev1.Node{}
				err = cl.Get(context.TODO(), client.ObjectKey{Name: "worker-1"}, updatedNode)
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedNode.Labels).NotTo(HaveKey(labelNodeRoleControlPlane))
			...
			It("Should not remove control-plane label with different value", func() {
				workerNode := &corev1.Node{
					ObjectMeta: metav1.ObjectMeta{
						Name: "worker-1",
						Labels: map[string]string{
							labelNodeRoleWorker:       "",
							labelNodeRoleControlPlane: "different-value",
						},
					},
				}
				...
				// Create a custom function that tries to remove the label but only if it matches
				removeLabelFunc := func(ctx context.Context, cli client.Client, nodeName string, logger logr.Logger) error {
					node := &corev1.Node{}
					if err := cli.Get(ctx, client.ObjectKey{Name: nodeName}, node); err != nil {
						return err
				...
				// Verify the label with different value was not removed
				updatedNode := &corev1.Node{}
				err = cl.Get(context.TODO(), client.ObjectKey{Name: "worker-1"}, updatedNode)
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedNode.Labels[labelNodeRoleControlPlane]).To(Equal("different-value"))
			})
			...
			It("Should label all nodes at startup", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedWorker1.Labels[labelNodeRoleControlPlane]).To(Equal(hypershiftLabelValue))

				updatedWorker2 := &corev1.Node{}
				err = cl.Get(context.TODO(), client.ObjectKey{Name: "worker-2"}, updatedWorker2)
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedWorker2.Labels[labelNodeRoleControlPlane]).To(Equal(hypershiftLabelValue))
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(updatedCP.Labels[labelNodeRoleControlPlane]).To(Equal(""))
			})
		})
