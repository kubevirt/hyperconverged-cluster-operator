...
Version: 1.2.3`)
		...
		Context("Rollout Strategy", func() {
			It("should be set to live update", func() {
				...
				Expect(kv.Spec.Configuration.VMRolloutStrategy).NotTo(BeNil())
				Expect(*kv.Spec.Configuration.VMRolloutStrategy).To(Equal(kubevirtcorev1.VMRolloutStrategyLiveUpdate))
			})
		})
