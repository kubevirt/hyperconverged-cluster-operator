	...
	When("enableHigherDensityWithSwap FG is set", func() {
		...
		Context("should create if not present - ", func() {
			...
			It("wasp-agent ClusterRole", func() {
				...
				Expect(cl.Get(context.Background(), client.ObjectKey{Name: res.Name}, foundClusterRole)).To(Succeed())
				Expect(expectedClusterRole.Name).To(Equal(foundClusterRole.Name))
				Expect(expectedClusterRole.Labels).To(HaveKeyWithValue(hcoutil.AppLabel, commontestutils.Name))
			})
			It("wasp-agent ClusterRoleBinding", func() {
				...
				Expect(cl.Get(context.Background(), client.ObjectKey{Name: res.Name}, foundClusterRoleBinding)).To(Succeed())
				Expect(expectedClusterRoleBinding.Name).To(Equal(foundClusterRoleBinding.Name))
				Expect(expectedClusterRoleBinding.Labels).To(HaveKeyWithValue(hcoutil.AppLabel, commontestutils.Name))
			})
		})
	...
	When("enableHigherDensityWithSwap FG is unset", func() {
		Context("should do nothing if not present - ", func() {
			...
			It("wasp-agent ClusterRole", func() {
				...
				Expect(cl.List(context.Background(), foundClusterRole)).To(Succeed())
				Expect(foundClusterRole.Items).To(BeEmpty())
			})
			It("wasp-agent ClusterRoleBinding", func() {
				cl = commontestutils.InitClient([]client.Object{hco})
				handler := newWaspClusterRoleBindingHandler(cl, commontestutils.GetScheme())
				...
				Expect(cl.List(context.Background(), foundClusterRoleBinding)).To(Succeed())
				Expect(foundClusterRoleBinding.Items).To(BeEmpty())
			})
		})
		Context("should remove if present - ", func() {
			...
			It("wasp-agent ClusterRole", func() {
				...
				res := handler.ensure(req)
				Expect(res.Err).ToNot(HaveOccurred())
				Expect(res.Name).To(Equal(newWaspClusterRole.Name))
				Expect(res.Created).To(BeFalse())
				Expect(res.Updated).To(BeFalse())
				Expect(res.Deleted).To(BeTrue())
			...
			It("wasp-agent ClusterRoleBinding", func() {
				...
				res := handler.ensure(req)
				Expect(res.Err).ToNot(HaveOccurred())
				Expect(res.Name).To(Equal(newWaspClusterRoleBinding.Name))
				Expect(res.Created).To(BeFalse())
				Expect(res.Updated).To(BeFalse())
				Expect(res.Deleted).To(BeTrue())
