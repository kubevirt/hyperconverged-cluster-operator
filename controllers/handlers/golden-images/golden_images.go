...
func init() {
	...
	if err = readDataImportCronTemplatesFromFile(os.DirFS(pwd)); err != nil {
		panic(fmt.Errorf("can't process the data import cron template file; %s; %w", err.Error(), err))
	}
}

func readDataImportCronTemplatesFromFile(myfs fs.FS) error {
	dataImportCronTemplateHardCodedMap = make(map[string]hcov1beta1.DataImportCronTemplate)

	if err := util.ValidateManifestDir(dataImportCronTemplatesFileLocation, myfs); err != nil {
		return errors.Unwrap(err) // if not wrapped, then it's not an error that stops processing, and it returns nil
	}

	return fs.WalkDir(myfs, dataImportCronTemplatesFileLocation, func(filePath string, d fs.DirEntry, internalErr error) error {
		if internalErr != nil {
			return internalErr
		}
