...
var _ = Describe("TLS Security Profile", Label("tls"), Serial, Ordered, func() {
	...
	AfterEach(func(ctx context.Context) {
		waitForSSPReady(ctx, cli, k8sCli)
	})
	...
	It("should enforce Custom TLS profile", func(ctx context.Context) {
		...
		minVersion := crypto.TLSVersionOrDie(string(customSpec.MinTLSVersion))
...
// TLS 1.3 cipher suites are not configurable in Go's crypto/tls and are
// always enabled, so they must be filtered before converting to cipher IDs.
...
func isTLSVersionAccepted(addr string, version uint16) (bool, error) {
	// Verify TCP connectivity first to distinguish transport errors from TLS rejections
	tcpConn, err := net.DialTimeout("tcp", addr, tlsDialTimeout)
	if err != nil {
...
func verifyTLSConfig(g Gomega, addr string, minVersion uint16, allowedTLS12Ciphers []uint16) {
	for _, v := range []uint16{tls.VersionTLS10, tls.VersionTLS11, tls.VersionTLS12} {
		if v < minVersion {
			accepted, err := isTLSVersionAccepted(addr, v)
			g.Expect(err).NotTo(HaveOccurred(),
				"transport error while checking TLS version 0x%04x", v)
			g.Expect(accepted).To(BeFalse(),
				"TLS version 0x%04x should be rejected (min version is 0x%04x)", v, minVersion)
		}
	}

	accepted, err := isTLSVersionAccepted(addr, tls.VersionTLS13)
	g.Expect(err).NotTo(HaveOccurred(), "transport error while checking TLS 1.3")
	...
	if minVersion <= tls.VersionTLS12 {
		...
		allowedSet := make(map[uint16]struct{}, len(allowedTLS12Ciphers))
		for _, c := range allowedTLS12Ciphers {
			allowedSet[c] = struct{}{}
		}

		for _, c := range serverCiphers {
			g.Expect(allowedSet).To(HaveKey(c),
...
func waitForSSPReady(ctx context.Context, cli client.Client, k8sCli *kubernetes.Clientset) {
	GinkgoHelper()
