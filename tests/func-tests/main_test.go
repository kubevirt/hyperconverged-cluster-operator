...
var _ = BeforeSuite(func() {
	...
	if jobType == "prow" {
		kubevirtCfg, err := virtCli.CoreV1().ConfigMaps(kvtestsflags.KubeVirtInstallNamespace).Get(tests.KubevirtCfgMap, metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred())
		kubevirtCfg.Data["debug.useEmulation"] = "true"
		_, err = virtCli.CoreV1().ConfigMaps(kvtestsflags.KubeVirtInstallNamespace).Update(kubevirtCfg)
		Expect(err).ToNot(HaveOccurred())
	}
})
