...
import (
	...
	kvtestsflags "kubevirt.io/kubevirt/tests/flags"
)
...
var _ = Describe("Unprivileged tests", func() {
	...
	It("should be able to read kubevirt-storage-class-defaults ConfigMap", func() {
		...
		_, err = unprivClient.CoreV1().ConfigMaps(kvtestsflags.KubeVirtInstallNamespace).Get("non-existent-configmap", metav1.GetOptions{})
		Expect(apierrors.IsForbidden(err)).To(BeTrue())

		configmap, err := unprivClient.CoreV1().ConfigMaps(kvtestsflags.KubeVirtInstallNamespace).Get("kubevirt-storage-class-defaults", metav1.GetOptions{})
