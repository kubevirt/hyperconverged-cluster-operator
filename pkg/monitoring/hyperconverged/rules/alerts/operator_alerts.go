...
func operatorAlerts() []promv1.Rule {
	return []promv1.Rule{
		...
		},
		{
			...
			Annotations: map[string]string{
				"description": "The cluster has no nodes with architecture matches any of the architectures of the {{ $labels.data_import_cron_name }} DataImportCronTemplate (for the {{ $labels.managed_data_source_name }} DataSource). This golden image will not be available for use in the cluster.",
