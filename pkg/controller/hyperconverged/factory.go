...
func (r *ReconcileHyperConverged) getAllResources(cr *hcov1alpha1.HyperConverged, request reconcile.Request) []runtime.Object {
	return []runtime.Object{
		...
		newIMSConfig(cr, request.Namespace),
	}
}
...
func newIMSConfig(cr *hcov1alpha1.HyperConverged, namespace string) *corev1.ConfigMap {
	labels := map[string]string{
		"app": cr.Name,
	}
	return &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "ims-config-name",
			Labels:    labels,
			Namespace: namespace,
		},
		Data: map[string]string{
			"imageA": "registry:project:repository",
			"tagA":   "tag",
			"imageB": "registry:project:repository",
			"tagB":   "tag",
		},
	}
}
