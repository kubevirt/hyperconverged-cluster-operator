...
func NewOperands(c client.Client, s *runtime.Scheme, eventEmitter hcoutil.EventEmitter, isOpenshift bool) Operands {
	operands := Operands{
		NewKvPriorityClassHandler(c, s, eventEmitter),
		newKubeVirtHandler(c, s, eventEmitter),
		newCdiHandler(c, s, eventEmitter),
		newNetworkAddonsHandler(c, s, eventEmitter),
		newVMImportHandler(c, s, eventEmitter),
	}

	if isOpenshift {
		operands = append(operands, Operands{
			newCommonTemplateBundleHandler(c, s, eventEmitter),
			newNodeLabellerBundleHandler(c, s, eventEmitter),
			newTemplateValidatorHandler(c, s, eventEmitter),
			newMetricsAggregationHandler(c, s, eventEmitter),
		}...)
	}
...
func (o Operands) Ensure(req *common.HcoRequest) error {
...
// ========== Common functions to all the operands ==========
