...
var _ = Describe("Downloads Service", func() {
	Context("Downloads Service", func() {
		...
		It("should create if not present", func() {
			expectedResource := NewDownloadsService(hco)
			cl := commonTestUtils.InitClient([]runtime.Object{})
			handler := (*genericOperand)(newDownloadsServiceHandler(cl, commonTestUtils.GetScheme()))
			res := handler.ensure(req)
			Expect(res.Err).To(BeNil())
...
var _ = Describe("Downloads Route", func() {
	Context("Downloads Route", func() {
		...
		It("should create if not present", func() {
			expectedResource := NewDownloadsRoute(hco)
			cl := commonTestUtils.InitClient([]runtime.Object{})
			handler := (*genericOperand)(newDownloadsRouteHandler(cl, commonTestUtils.GetScheme()))
			res := handler.ensure(req)
			Expect(res.Err).To(BeNil())
