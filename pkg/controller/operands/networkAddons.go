...
type networkAddonsHandler struct {
	client       client.Client
	scheme       *runtime.Scheme
	eventEmitter hcoutil.EventEmitter
}

func newNetworkAddonsHandler(c client.Client, s *runtime.Scheme, ee hcoutil.EventEmitter) *networkAddonsHandler {
	return &networkAddonsHandler{
		client:       c,
		scheme:       s,
		eventEmitter: ee,
	}
...
func (na networkAddonsHandler) ensure(req *common.HcoRequest) *EnsureResult {
	...
	err = na.client.Get(req.Ctx, key, found)

	if err != nil {
		if apierrors.IsNotFound(err) {
			req.Logger.Info("Creating Network Addons")
			err = na.client.Create(req.Ctx, networkAddons)
			if err == nil {
				na.eventEmitter.EmitCreatedEvent(req.Instance, getResourceType(networkAddons), key.Name)
	...
	if len(existingOwners) > 0 {
		...
		err = na.client.Update(req.Ctx, found)
		if err != nil {
	...
	if !reflect.DeepEqual(found.Spec, networkAddons.Spec) && !req.UpgradeMode {
		...
		na.eventEmitter.EmitUpdatedEvent(req.Instance, getResourceType(networkAddons), key.Name)
		return res.SetUpdated()
	}
	...
	objectRef, err := reference.GetReference(na.scheme, found)
	if err != nil {
		return res.Error(err)
	}
