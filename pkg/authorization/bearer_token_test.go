...
var _ = Describe("BearerToken", func() {
	BeforeEach(func() {
		tmpFile, err := os.CreateTemp("", "fake-token")
		Expect(err).ToNot(HaveOccurred())

		_, err = tmpFile.Write([]byte("test-secret-key"))
		Expect(err).ToNot(HaveOccurred())

		os.Setenv(authorization.TokenPathEnvVar, tmpFile.Name())
	})

	AfterEach(func() {
		tokenPath := os.Getenv(authorization.TokenPathEnvVar)
		if tokenPath != "" {
			os.Remove(tokenPath)
		}
	...
	Context("with a valid token", func() {
		It("should create and validate a token", func() {
