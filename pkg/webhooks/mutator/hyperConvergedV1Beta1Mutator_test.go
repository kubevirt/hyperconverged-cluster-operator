...
var _ = Describe("test HyperConverged mutator", func() {
	...
	Expect(hcov1beta1.AddToScheme(mutatorScheme)).To(Succeed())
	BeforeEach(func() {
		...
		mutator = initV1Beta1HCMutator(mutatorScheme, cli)
	})
	...
	Context("Check mutating webhook for create operation", func() {
		...
		It("should handle multiple DICTs and mediatedDevicesTypes -> mediatedDeviceTypes at the same time", func() {
			...
			cr.Spec.MediatedDevicesConfiguration = &hcov1beta1.MediatedDevicesConfiguration{
		...
		DescribeTable("Check mediatedDevicesTypes -> mediatedDeviceTypes transition", func(initialMDConfiguration *hcov1beta1.MediatedDevicesConfiguration, patches []jsonpatch.JsonPatchOperation) {
...
func initV1Beta1HCMutator(s *runtime.Scheme, testClient client.Client) *HyperConvergedV1Beta1Mutator {
