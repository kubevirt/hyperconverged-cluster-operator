...
| Field | Description | Scheme | Default | Required |
| ----- | ----------- | ------ | -------- |-------- |
| downwardMetrics | Allow to expose a limited set of host metrics to guests. Deprecated: to set this feature gate in KubeVirt, add \"DownwardMetrics\" to the /spec/kubevirtFeatureGate list | *bool | false | false |
...
| kubevirtFeatureGates | KubeVirtFeatureGates is a list of feature gates to set in the KubeVirt custom resource In order to remove a feature gate that was added by HCO, add the feature gate name with add a proceeding \"!\" to the feature gate name; e.g. \"!GPU\" | KubeVirtFeatureGates |  | false |
...
| kubevirtFeatureGates | KubeVirtFeatureGates is a list of feature gates that HCO set in the KubeVirt custom resource, including the feature gates set by the HCO code, and the once that was added from the spec.kubevirtFeatureGates field. | KubeVirtFeatureGates |  | false |
