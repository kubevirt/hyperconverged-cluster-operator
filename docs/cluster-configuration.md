...
## Introduction
The HyperConverged CR (Custom Resource) allows modifying the KubeVirt cluster configuration by editing the CR fields.
There is only one instance of this CR, and it always in the installation namespace, and it's name is
`kubevirt-hyperconverged`.

The HyperConverged Cluster operator copies the cluster configuration values to the other operand's CRs.

This document is the KubeVirt cluster configuration user guide, and it makes use of the new `v1` API. This new API
is not mandatory at this point, but in a future release it will replace the current `v1beta1` API version, that will be
dropped, eventually.

### Manually editing other CRs
The Hyperconverged Cluster Operator configures KubeVirt and its supporting operators in an opinionated way and reconciles its operands when there is an unexpected change to them.
...
available yet.
Add a new FeatureGate Object to this set, to enable a feature that is disabled by default, or to disable a feature that
is enabled by default.

A feature gate may be in the following phases:
* `alpha`: the feature is in Developer Preview. It is disabled by default, but
  can be enabled. 
  
  > *Note*: Features in Alpha phase may be canceled and removed.  

* `beta`: the feature gate is in Technology Preview. It is enabled by default,
  but can be disabled.

  > *Note*: Features in Beta mode are candidates for graduation, but even if
    is not common, they may still be canceled. 

* `GA` (Generally Available): the feature is graduated and is always enabled.
  There is no way to disable it.

* `deprecated`: the feature is no longer supported. There is no way to enable
  it.

To enable a feature, add a new feature gate object to the `featureGates` list. To disable a feature gate that is enabled
...
Decentralized live migration is enabled by default. To disable it, add the `decentralizedLiveMigration` feature gate
with the `state` field of `Disabled`.
...
Add the `declarativeHotplugVolumes` feature gate, to enable the declarative volume hotplug API in KubeVirt. By default,
volume hotplug operations are performed using KubeVirt's subresource API. Changes made directly to the VirtualMachine
...
objects that are related to the specified VM or VMI, enabling better dependency
tracking.
...
Add the `incrementalBackup` feature gate in order to enable changed block
tracking and incremental backups using QEMU capabilities in KubeVirt.

Enabling changed block tracking is mandatory for performing storage-agnostic
backups and incremental backups.

When enabled, this also enables the `UtilityVolumes` feature gate in the
KubeVirt CR, which allows utility volumes to be mounted to the VMI
virt-launcher pod without having a matching disk in the domain. This is
...
Add the `containerPathVolumes` feature true in order to enable the use of
container paths as volumes in KubeVirt. This allows VMs to access files and
directories from the virt-launcher pod's filesystem via virtiofs. This is
useful for exposing data dynamically injected into pods by Kubernetes or
platform-specific mechanisms, such as projected service account tokens for
workload identity.
...
SEV-SNP workload encryption for confidential virtual machines (VMs). When
enabled, this allows deploy VMs in which memory and CPU registers are encrypted
...
workload encryption for confidential virtual machines (VMs). When enabled, this
allows deploy VMs in which memory and CPU registers are encrypted and isolated
from the host.
...
The `spec.virtualization` field contains all the configurations for virtualization.

### Live Migration Configurations

Set the live migration configurations by modifying the fields in the `liveMigrationConfig` under the `spec.virtualization` field

#### bandwidthPerMigration

Bandwidth limit of each migration, the value is in quantity of bytes per second. The format is a number with an optional quantity suffix, e.g. `2048Mi` means 2048MiB/sec.
...
Number of migrations running in parallel in the cluster. The format is an integer number.
...
Cluster-admins can provide a list of desired mediated devices (vGPU) types.
...
NodeSelectors to target a specific node.
...
mediated device names. Use the `spec.virtualization.permittedHostDevices` field in order to manage the permitted host devices.
...
* `selectors` array is an array of `USBSelector` objects. The fields of this object are a **`vendor`, `product`** required to identify a USB device.
  ...
  The two identifiers like `06cb` and `00f9` can be found using `lsusb`; for example:
   ...
   ```
  Multiple `USBSelector` objects can be used to match a set of devices.
...
### Resource Requests: VMI PODs CPU Allocation Ratio
...
### Resource Requests: Storage Resource Configurations

The administrator can limit storage workloads resources and to require minimal resources.
Add the `workloadResourceRequirements` field under the `spec.storage`. The
content of the `workloadResourceRequirements` field is
...
Use the `spec.virtualization.obsoleteCPUModels` to configure the CPU plugin. Add the obsolete CPU list under `spec.obsoleteCPUs.cpuModels`.

The default value for the `spec.virtualization.obsoleteCPUModels` field is hardcoded predefined list and is not visible. You can
...
The hard-coded predefined list of obsolete CPU models is:
* `486`
* `pentium`
* `pentium2`
* `pentium3`
...
You don't need to add a CPU model to the `spec.virtualization.obsoleteCPUModels` field if it is in this list.
...
not set too, host-model will be set.
...
us the HyperConverged `spec.virtualization.workloadUpdateStrategy` object to define how to handle automated workload
updates at the cluster level.
...
This configuration requires the `workloadEncryptionTDX` feature gate to be
enabled, and the host must have Intel TDX capable hardware with the appropriate
kernel parameters (`kvm_intel.tdx=1`, `nohibernate`).
...
the `spec.virtualization.vmStateStorageClass` field defines the [Kubernetes Storage Class](https://kubernetes.io/docs/concepts/storage/storage-classes/)
...
The `spec.virtualization.autoCPULimitNamespaceLabelSelector` field allows defining a namespace label for which VM pods
(virt-launcher) will have a CPU resource limit of 1 per vCPU.
...
limit of 1 per vCPU.
**Important note**: this setting is incompatible with a `vmiCPUAllocationRatio` of 1, since that configuration can lead
to VM pods using more than 1 CPU per vCPU.

### Virtual machine options

The `spec.virtualization.virtualMachineOptions` field holds the cluster level information regarding the virtual machine.
...
> However, if you type by mistake a password for a command or if you have to pass it as a clear text parameter it will
> be written in the logs along with all other visible text.
>
> If you are inputting any data or commands that contain sensitive information please consider using SSH unless the
> serial console is absolutely necessary for troubleshooting.
...
User can specify a cluster-wide default RuntimeClass for VMIs pods: default RuntimeClass is set when vmi doesn't have
any specific RuntimeClass.

When vmi RuntimeClass is set, then vmi's RuntimeClass is preferred. When default RuntimeClass is not set and vmi's
RuntimeClass is not set too, RuntimeClass will not be configured on VMIs pods.

Default RuntimeClass can be changed when kubevirt is running, existing VMIs are not impacted till the next
restart or live-migration when they are eventually going to consume the new default RuntimeClass.
```yaml
apiVersion: hco.kubevirt.io/v1
kind: HyperConverged
metadata:
...
The `spec.virtualization.ksmConfiguration` instructs on which nodes KSM will be enabled, exposing a `nodeLabelSelector`.
...
- Enabling KSM on nodes with labels `kubevirt.io/first-label: true` **and** `kubevirt.io/second-label: true`:
```yaml
apiVersion: hco.kubevirt.io/v1
kind: HyperConverged
metadata:
...
**Note**: When updating the overcommit percentage, changes will apply to existing VM workloads only after a power cycle or after live-migration.
...
> **_Note_:**  HCO will not configure the rate limiters if both the annotation and the `spec.virtualization.tuningPolicy` are not populated correctly.
...
```bash
kubectl patch -n kubevirt-hyperconverged hco kubevirt-hyperconverged --type=json -p='[{"op": "add", "path": "/spec/virtualization/tuningPolicy", "value": "annotation"}]'
```

> **_Note_**: the `HighBurst` profile that was deprecated in API v1beta1, was dropped in API v1, and is not supported.

### Role Aggregation Strategy

By default, KubeVirt aggregates its RBAC ClusterRoles (`kubevirt.io:admin`, `kubevirt.io:edit`, `kubevirt.io:view`) into
the corresponding Kubernetes default roles (`admin`, `edit`, `view`). This means that any user with the Kubernetes
`admin` role automatically gets permissions to manage KubeVirt resources.

The optional `spec.virtualization.roleAggregationStrategy` field allows cluster administrators to disable this
aggregation. When set to `Manual`, the `aggregate-to-*` labels on KubeVirt ClusterRoles are being set to `false`,
and administrators must explicitly create RoleBindings to grant access to KubeVirt resources. When set to
`AggregateToDefault` or left unset, the default aggregation behavior is preserved. Changes to this field can be made at
any time and are propagated to the KubeVirt CR at runtime.

This feature requires the `OptOutRoleAggregation` feature gate, which is automatically added to the KubeVirt CR when the
`roleAggregationStrategy` field is set in the HyperConverged custom resource.
...
In order to configure the live upgrade of virtual machines, you can set the optional
`spec.virtualization.liveUpdateConfiguration` object in the HyperConverged CR. See more details in the
[KubeVirt documentation](https://kubevirt.io/user-guide/compute/cpu_hotplug/#optional-set-maximum-sockets-or-hotplug-ratio).

The `spec.virtualization.liveUpdateConfiguration` field is with the same structure as the corresponding field in the
[KubeVirt CR](https://kubevirt.io/api-reference/main/definitions.html#_v1_liveupdateconfiguration).
...
By default, KubeVirt uses KVM as its hypervisor. The `spec.virtualization.hypervisors` field allows cluster
administrators to specify an alternative hypervisor to use for running virtual machines. This is useful for environments
where a different hypervisor is available on the cluster nodes, such as Hyper-V Direct.

When `spec.virtualization.hypervisors` is set, the HyperConverged operator automatically enables the
`ConfigurableHypervisor` feature gate in the KubeVirt CR. If `spec.virtualization.hypervisors` is empty or not set, the
feature gate is not added and KubeVirt uses the default KVM hypervisor.
...
The `spec.storage` field contains all the configurations for storage.
...
`scratchSpaceStorageClass` field under the HyperConverged `spec.storage` field.
...
spec:
  storage:
    scratchSpaceStorageClass: my-storage-class
```
...
`insecureRegistries` field under the `storageImport` in the `HyperConverged`'s `spec.storage` field.
...
You can configure certificate rotation parameters to influence the frequency of the rotation of the certificates needed
by a Kubevirt deployment.

All values must be 10 minutes in length or greater to avoid overloading the system, and should be expressed as strings
that comply with [golang's ParseDuration format](https://golang.org/pkg/time/#ParseDuration).
...
Starting from OCP/OKD 4.6, a [cluster-wide API](https://github.com/openshift/enhancements/blob/master/enhancements/kube-apiserver/tls-config.md) is available for cluster administrators to set TLS profiles for
OCP/OKD core components.

HCO, as an OCP/OKD layered product, will follow along OCP/OKD crypto policy cluster-wide setting, and use the same
profile configured for the cluster’s control plane.

Configuration of a TLS security profile ensures that OCP/OKD, as well as HCO and its sibling operators and operands, use
cryptographic libraries that do not allow known insecure protocols, ciphers, or algorithms.

By default, on OCP/OKD, HCO will read the global configuration for TLS security profile of the APIServer, without
storing it in HCO CR, and will propagate the .spec.tlsSecurityProfile stanza to all underlying HCO managed custom
resources.
  ...
  networking:
    networkBinding:
      ...
      custom-binding2:
        ...
        networkAttachmentDefinition: custom-binding2nad
```
...
In order to set KubeMacPool MAC ranges, configure them on the HyperConverged CR under the
`spec.networking.kubeMacPoolConfiguration` field.
...
**Note**: You must configure both `rangeStart` and `rangeEnd` together. Partial configuration (only one field) is not
supported, and will be rejected.

## Workload Sources Configurations
The `spec.workloadSources` field contains all the configurations for workload sources.
...
### Modify pre-defined (common) golden images
Golden images are root disk images for commonly used operating systems. HCO provides several pre-defined images, but it is
possible to modify them, if needed.

The list of all the golden images is available at the `status` field, under `dataImportCronTemplates` list. The
pre-defined images are not part of list in the `spec` field. The list in the status is a reference for modifications.
Add the image that requires modification to the list in the spec, and edit it.
...
#### Disabling all pre-defined golden images

Set the `spec.workloadSources.enableCommonBootImageImport` to `false` in order to disable the common golden images in the cluster
...
**Note**: `spec.workloadSources.enableCommonBootImageImport` is `true` by default.
...
#### Disabling a specific pre-defined golden image
To disable a golden image, add it to the `dataImportCronTemplates` field, under the `spec.workloadSources` field, with
the `dataimportcrontemplate.kubevirt.io/enable` annotation, with the value of `false`.

For example, disabling the fedora golden image:
...
There is no need to copy the whole object, but only the relevant fields; i.e. the `metadata.name` field.

#### Modifying a pre-defined dataImportCronTemplate
It is possible to change the configuration of a pre-defined golden image by adding the common image to the
`dataImportCronTemplates` list in the `spec.workloadSources:` field. HCO will replace the existing spec object;
The `schedule` field is mandatory, and HCO will copy it from the pre-defined template if it is missing.

Copy the required dataImportCronTemplate object from the list in the `status` field (not including the `status`
...
To use another namespace for the pre-defined golden images, e.g. in order to hide them, set the
`spec.workloadSources.commonBootImageNamespace` field with the required namespace name.
...
the `HyperConverged`'s `spec.workloadSources` field.
...
**Note**: even if the `spec.workloadSources.enableCommonBootImageImport` field is set to `false`, it does not block the
custom golden images, but only the pre-defined ones.
      ...
      spec:
        ...
        template:
          spec:
            source:
              registry:
                url: docker://myprivateregistry/custom1:v8
      ...
      spec:
        ...
        template:
          spec:
            source:
              registry:
                url: docker://myprivateregistry/custom2:v10
...
##### Modifying a pre-defined Golden Image in Heterogeneous Cluster
When [modifying a pre-defined golden image](#modify-pre-defined-common-golden-images), if the change contains a change in the image
source; i.e. the image is not the same as the original pre-defined golden image, then the modified DataImportCronTemplate
  ...
  workloadSources:
    ...
    - metadata:
      ...
      spec:
        ...
        managedDataSource: centos-stream8
```
If the image source is the same as the original pre-defined golden image, then the annotation is not required, and if it
...
The configuration of [instance type and preference related features](https://kubevirt.io/user-guide/user_workloads/instancetypes/) within KubeVirt can be configured through the
use of the `spec.workloadSources.instancetypeConfig` configurable. This field matches the field provided by the
[KubeVirt CR](https://kubevirt.io/api-reference/main/definitions.html#_v1_instancetypeconfiguration).
...
The configuration of [common instance types deployment](https://kubevirt.io/user-guide/user_workloads/deploy_common_instancetypes/) is held separately to the core instance type and preference
functionality within the [KubeVirt CR](https://kubevirt.io/api-reference/main/definitions.html#_v1_commoninstancetypesdeployment). This is mirrored in the HCO CR through the
`spec.workloadSources.commonInstancetypesDeployment` configurable.
...
Different levels of verbosity are used in CDI to control the amount of information that is logged. The verbosity level
of logs in CDI can be adjusted using a dedicated field that affect all of its components.
This feature enables users to control the amount of detail displayed in logs, ranging from minimal to detailed debugging
information.
...
The verbosity levels in CDI typically range from 1 to 3. Level 1 equates to essential log information, while level 3
delves into more detailed logging, providing higher verbosity.

### Workloads protection on uninstall

The `spec.deployment.uninstallStrategy` field defines how to proceed on uninstall when workloads (`VirtualMachines`,
...
To enable the AAQ support, set the `spec.deployment.applicationAwareConfig.enable` field to `true`.

To configure AAQ, set the fields of the `applicationAwareConfig` object in the HyperConverged resource's
`spec.deployment` field. The `applicationAwareConfig` object contains several fields:
...
In order to allow access to the Virtual Machine's VNC Console, set the `spec.deployment.deployVmConsoleProxy` field to `true`.
...
##### Disable KubeMacPool
If KubeMacPool is buggy on your cluster and you do not immediately need it you can configure CNAO to not deploy it.
```bash
