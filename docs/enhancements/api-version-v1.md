...
* `evictionStrategy`
* `virtualMachineOptions`; see [VirtualMachineOptions](#VirtualMachineOptions) section below, for changes
  to this field
...
* From the `resourceRequirements` field:
    ...
    **note**: the `resourceRequirements.storageWorkloads` field will be moved to under
              new `spec.storage` field; see [Storage](#storage) section below.
...
`kubeSecondaryDNSNameServerIP` stays on v1beta1 as an ignored field. It is not
converted to v1.
