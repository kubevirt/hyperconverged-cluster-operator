...
* Using `docker run`
    ```shell
     docker run --env KUBECONFIG=/tmp/conf \
      ...
      quay.io/kubevirt/hyperconverged-cluster-functest:1.17.0-unstable 
    ```
...
kubectl run functest --image=quay.io/kubevirt/hyperconverged-cluster-functest:1.17.0-unstable --serviceaccount=functest
```
#### OpenShift
```shell
