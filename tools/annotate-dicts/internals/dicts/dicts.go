...
import (
	...
	"github.com/kubevirt/hyperconverged-cluster-operator/tools/annotate-dicts/internals/cleanup"
	"github.com/kubevirt/hyperconverged-cluster-operator/tools/annotate-dicts/internals/images"
)
...
func (d *Dicts) updateAnnotation(ctx context.Context, dict *hcov1beta1.DataImportCronTemplate, url string) func() error {
	return func() error {
		...
		if arches == "" {
			log.Printf("No architectures found for %s", url)
			return nil
		}
