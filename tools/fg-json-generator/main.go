...
func phaseFromHcoAnnotation(commentText string) (featuregates.Phase, bool) {
	...
	if !strings.HasPrefix(text, hcoFgPhaseMarker) {
		return featuregates.PhaseUnknown, false
	}

	phase := strings.TrimPrefix(text, hcoFgPhaseMarker)
	switch phase {
	case "discontinued":
...
// fieldPhase determines the lifecycle phase of a feature gate field from
// // +hco:fgphase:... when present; otherwise from the kubebuilder default value.
