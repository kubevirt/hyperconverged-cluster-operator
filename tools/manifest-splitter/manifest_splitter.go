...
var (
	unknownFileType = newFileTypeMultiFiles("unknown")

	fileTypeMap = fileTypes{
		types: map[string]fileType{
			"kind: ClusterServiceVersion":    &csvFileType{},
			"kind: CustomResourceDefinition": newFileTypeMultiFiles("crd"),
			"kind: NetworkPolicy":            newFileTypeMultiFiles("networkpolicy"),
		},
	}
)
...
func splitFile(fin io.Reader, operatorName, outputDir string) error {
	...
	fh := newFileHandler(operatorName, unknownFileType, outputDir)
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, kindLinePrefix) {
			...
			fh.setFileType(fileTypeMap.getFileType(line))
		}
	}
